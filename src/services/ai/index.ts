...
// ── Topic extraction (knowledge graph) ──────────────────────────────────────

export interface ExtractedTopic {
  /** Human-readable topic label, e.g. "Q3 renewal", "onboarding". */
  name: string
  /** Lowercased normalized dedupe key. Advisory only — graph.service re-derives
   *  the authoritative key from `name` so extraction and backfill agree. */
  canonicalKey: string
}
...
export interface ExtractTopicsPayload {
  ...
  contactName: string
  messages: { direction: 'INBOUND' | 'OUTBOUND'; content: string }[]
  /** The user's existing top topics (by weight) — fed to the model so it reuses
   *  a matching one instead of minting near-duplicate wording. */
  existingTopics: { name: string; canonicalKey: string }[]
}

const TOPICS_SCHEMA: AiJsonSchema = {
  type: 'object',
  properties: {
    topics: {
      type: 'array',
      items: {
        ...
        properties: {
          name: {
            type: 'string',
            description:
              'A short, reusable topic label (1–3 words), e.g. "Q3 renewal", "pricing", "onboarding". Prefer a concrete business subject over generic filler like "email" or "follow up".',
          },
          canonicalKey: {
            type: 'string',
            description:
              'The topic lowercased with punctuation stripped and spaces collapsed, e.g. "q3 renewal".',
          },
        },
        required: ['name', 'canonicalKey'],
      },
      description: '1–3 distinct topics this conversation is about. Fewer is better than forcing three.',
    },
  },
  required: ['topics'],
}

/** Advisory topic-key normalization used inside extractTopics. The AUTHORITATIVE
 *  key is re-derived by graph.service's own normalizer at upsert time, so this
 *  only needs to be a reasonable hint. Kept in sync intentionally. */
...
function buildTopicsPrompt(p: ExtractTopicsPayload): string {
  const thread = p.messages
    .map((m) => {
      const role = m.direction === 'INBOUND' ? `THEM (${p.contactName})` : 'ME'
  ...
  const existing = p.existingTopics.length
    ? `\n\nEXISTING TOPICS (reuse the EXACT name + key of one of these when the conversation is about the same thing — do NOT invent near-duplicate wording like "pricing" vs "price"):\n${p.existingTopics
        .map((t) => `- ${t.name} (${t.canonicalKey})`)
        .join('\n')}`
    : ''

  return `Extract the key business topics discussed in this ${p.subject ? `email thread "${p.subject}"` : 'email thread'} with "${p.contactName}", for a relationship knowledge graph.
Return 1–3 distinct, reusable topic labels — the subjects a salesperson would file this conversation under (projects, products, deals, requests). Ignore pleasantries and signatures.
Respond with topics in the SAME LANGUAGE as the conversation.${existing}
...
/**
 * Extract 1–3 knowledge-graph topics from a conversation. Best-effort and
 * provider-gated (like parseSearchQuery): returns [] when no text provider is
 * available. Mirrors the analyze/draft fallback contract — retryable provider
 * errors are rethrown so the extraction job can back off for rate-limit pacing,
 * UNLESS `fallbackOnRetryable` is set (final job attempt), where it degrades to
 * [] (no topics this round; the deterministic company edge still stands).
 */
export async function extractTopics(
  payload: ExtractTopicsPayload,
  opts: { fallbackOnRetryable?: boolean } = {},
  ...
  try {
    const raw = await provider.generateJson<{ topics?: unknown }>({
      prompt: buildTopicsPrompt(payload),
    ...
    if (err instanceof AiProviderError && err.retryable && !opts.fallbackOnRetryable) throw err
    console.warn(`[ai] ${provider.name} topic extraction failed (${String(err)}); skipping topics this round`)
    return []
  }
}
