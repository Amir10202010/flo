...
function PlanCard({ plan, period }: { plan: BillingPlan; period: BillingPeriod }) {
  ...
  const isCheckout = c.href.startsWith('/api/billing/checkout')

  function onCtaClick() {
    track('plan_cta_clicked', { plan, period })
    // A checkout-bound CTA (any paid plan) also starts the money path.
    if (isCheckout) track('checkout_started', { plan, period })
  }
  ...
  return (
    ...
    >
      ...
      <Link href={c.href} onClick={onCtaClick} className={accent ? 'btn-primary' : 'btn-ghost'} style={{ justifyContent: 'center', textDecoration: 'none', marginBottom: 20, gap: 8 }}>
